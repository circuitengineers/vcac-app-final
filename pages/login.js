      ...
      <div style={{ width: '100%', maxWidth: 420, position: 'relative', zIndex: 1 }}>
        ...
        <div className="card" style={{ padding: '2rem' }}>
          ...
          <div style={{ display: 'flex', background: 'var(--panel)', borderRadius: 8, padding: 4, marginBottom: '1.5rem' }}>
            {['signin', 'signup'].map(m => (
              <button key={m} onClick={() => { setMode(m); setError(null); setMessage(null) }} style={{
                ...
                fontSize: '0.85rem', fontWeight: 500, fontFamily: 'DM Sans',
          ...
          <button onClick={handleGoogleLogin} disabled={loading} style={{
            ...
            background: 'var(--panel)', color: 'var(--text)', fontFamily: 'DM Sans',
          ...
          <form onSubmit={handleEmailAuth} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {mode === 'signup' && (
              <div>
                ...
                <input type="text" placeholder="vibemaster_99" value={username} onChange={e => setUsername(e.target.value)} required minLength={3} maxLength={30} />
                <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', marginTop: 4 }}>Lowercase letters, numbers, underscores only</div>
              </div>
            )}
            <div>
              <label>Email *</label>
